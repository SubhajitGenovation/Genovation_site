...
export default function PerfFooter() {
  ...
  const technology = [
    ...
    { label: "Security IP",  active: false },
    { label: "SLM Models",   active: false },
  ];
        ...
        <div className="grid md:grid-cols-5 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {platform.map((item) => (
                <a key={item} href="#" className="block text-[13px] text-gray-400 hover:text-gray-700 font-light transition-colors">{item}</a>
              ))}
            </div>
          </div>
          ...
          {/* Technology */}
          <div>
            <div className="text-[11px] text-gray-500 font-mono tracking-wider uppercase mb-4">Technology</div>
            <div className="space-y-2.5">
              {technology.map((item) => (
                <a key={item.label} href="#" className={`block text-[13px] font-light transition-colors ${item.active ? "text-emerald-600" : "text-gray-400 hover:text-gray-700"}`}>
                  {item.label}
                </a>
              ))}
            </div>
        ...
        <div className="border-t border-black/[0.04] pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-8">
            {["Privacy","Terms","Security"].map((item) => (
              <a key={item} href="#" className="text-xs text-gray-400 hover:text-gray-700 font-light transition-colors">{item}</a>
            ))}
          </div>
        </div>
      </div>
