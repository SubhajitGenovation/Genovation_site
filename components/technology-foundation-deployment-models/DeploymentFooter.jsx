...
const FOOTER_LINKS = {
  Platform: ["Mentis AgentOS", "JUDGE", "CipherVault", "SLM Engine"],
  Products: ["Knowledge", "Analytics", "Strategy", "CX"],
  Company: ["About", "Careers", "Security", "Contact"],
};

export default function DeploymentFooter() {
  return (
    <footer className="py-16" style={{ borderTop: "1px solid rgba(0,0,0,0.06)" }}>
      <div className="max-w-7xl mx-auto px-8">
        <div className="grid md:grid-cols-5 gap-12 mb-12">
          ...
          {Object.entries(FOOTER_LINKS).map(([category, links]) => (
            <div key={category}>
              <div className="text-[11px] text-gray-500 font-mono tracking-wider uppercase mb-4">{category}</div>
              <div className="space-y-2.5">
                {links.map((link) => (
                  <a key={link} href="#" className="block text-[13px] text-gray-500 hover:text-gray-700 font-light transition-colors">
                    {link}
                  </a>
                ))}
              </div>
        ...
        >
          ...
          <div className="flex gap-8">
            {["Privacy", "Terms", "Security"].map((item) => (
              <a key={item} href="#" className="text-xs text-gray-500 hover:text-gray-700 font-light transition-colors">
                {item}
              </a>
            ))}
          </div>
